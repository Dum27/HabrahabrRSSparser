...
public class RssItem implements Comparable<RssItem>, Parcelable
{
	...
	public RssItem(String title, String link, String day)
	{
		this.title = title;
		this.link = link;
		this.day = day;
	...
	public void setPubDate(String pubbDate) {
		try {
			SimpleDateFormat dateFormat = new SimpleDateFormat("EEE, dd MMM yyyy HH:mm:ss Z", Locale.ENGLISH);
		    this.pubDate = dateFormat.parse(pubbDate);
			Date today = new Date();
//			Log.i("TODAY DATE = = = ", String.valueOf(today.getDate()));
//			Log.i("NEWS = = = ", String.valueOf(pubDate.getDate()));
			if(pubDate.getDate()==today.getDate())
			{
				setDay("Today");
			}else
			if(pubDate.getDate()+1==today.getDate())
			{
				setDay("Yesterday");
			}else
			{
				setDay("Some days ago");
			}
